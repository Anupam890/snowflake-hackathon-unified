...
BEGIN
    ...
    ELSEIF (:v_intent = ''FORECAST'') THEN
        ...
        SELECT
            ...
            CHR(10) || CHR(10) ||
            ''| Policy Type | Month | Forecast New Policies | Lower Bound | Upper Bound | Assumed Premium |'' || CHR(10) ||
            ''|---|---|---|---|---|---|'' || CHR(10) ||
            LISTAGG(
                ''| '' || f.SERIES::VARCHAR ||
                '' | '' || TO_CHAR(f.TS, ''YYYY-MM'') ||
                '' | '' || GREATEST(ROUND(f.FORECAST, 0), 0)::VARCHAR ||
                '' | '' || GREATEST(ROUND(f.LOWER_BOUND, 0), 0)::VARCHAR ||
                '' | '' || GREATEST(ROUND(f.UPPER_BOUND, 0), 0)::VARCHAR ||
    ...
    ELSEIF (:v_intent = ''SHOW_RESULTS'') THEN
        ...
        ELSE
            SELECT
                ''LATEST FORECAST RESULTS'' || CHR(10) || CHR(10) ||
                ''| Policy Type | Month | Forecast New Policies | Lower Bound | Upper Bound | Assumed Premium |'' || CHR(10) ||
                ''|---|---|---|---|---|---|'' || CHR(10) ||
                LISTAGG(
                    ''| '' || POLICY_TYPE ||
                    '' | '' || TO_CHAR(FORECAST_MONTH, ''YYYY-MM'') ||
                    '' | '' || FORECAST_NEW_POLICIES::VARCHAR ||
                    '' | '' || LOWER_BOUND::VARCHAR ||
                    '' | '' || UPPER_BOUND::VARCHAR ||
                    '' | '' || ROUND(ASSUMED_AVG_PREMIUM, 2)::VARCHAR || '' |'',
                    ''\\n''
